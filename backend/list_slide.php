<?php
    ...
    try {
        $stmt = $connect->prepare("SELECT  id, capa, titulo, descricao FROM foto");
        $stmt->execute();

        $eventos = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $result["success"]["message"] = "Imagem listada com sucesso"; //criamos o array para devolver o resultado do insert numa mensagem de sucesso.
